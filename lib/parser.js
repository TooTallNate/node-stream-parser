...
function data (stream, chunk, output, fn) {
  /*if (stream.done) {
  ...
  if (chunk.length <= stream._parserBytesLeft) {
    ...
    if (chunk.length > b.length) {
      data(stream, chunk.slice(b.length), output, fn);
    }
  }
}
...
function process (stream, chunk, output) {
  ...
  if (0 === stream._parserBytesLeft) {
    ...
    if (cb) {
      if (chunk) {
        cb.call(stream, chunk, output); // buffered
      } else {
        cb.call(stream, output); // passthrough
      }
    }
    //assert(/*stream.done || */stream._parserBytesLeft > 0, 'buffer() or passthrough() were not called in the callback');
  }
}
